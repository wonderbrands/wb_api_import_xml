...
mycursor.execute("""select b.order_id, a.uuid, a.fecha
                    ...
                    where a.folio in
                        (373232601112,740236300337,297233100188,376232900257,377233801467,351234500597,377234001687,743236400681,373234300870,744235400212,378233001299,378234100456,378233100187,745234800588,379233900578,745236801724,743237000766,746235101890,357234203184,380233400021,746236500823,746236601007,745236801775,746234900328,747235301371,746237001658,381234002268,381234201349,381233700389,383232500783,749236600840,750234900473,750235100729,384233200071,384233901112,750236701980,384234600967,385232501014,385233800979,385234300895,374234201849,384234102343,752234800611,386234201602,752237001514,382234300153,753235500344,753236500500,387234001778,387234100074,753236501541,731236700973,753236700338,753236601965,387234402036,387234402300,387234402352,753236902505,387233400177,753237001645,754234901409,388232901314,388233900652,754236401591,388234201620,754235001706,389232601730,755237000665,390234002214,391233200509,391232501956,391233901370,757236300898,391234100865,391234101869,385234501844,391234001346,390234601215,392232502071,757234801152,392234001531,758236402726,392234500375,759234801888,759235101546,393232900707,759235400653,759235500314,393233900099,393233901232,759236400647,759236401639,759236600111,328234000243,393234600518,759235301327,394232400263,394232500921,394232601165,394232801402,394233600564,394234000087,760236700594,760236601288,760236700048,760236800213,760236700693,394234700265,394234605172,394234703178,395232404029,395232404063,395232601990,761235100662,761236100369,395233700646,395234200656,395234600115,762236400739,762236501692,396234100067,390234700644,763236600543,764235200078,398234100275,764236502020,764236600751,398234102649,764236801479,398234500627,764237001568,393232801516,393232501859,764236601292,765236601113,765236702257,399234601551,766234800697,766235200606,724236401889,386233700166,732235600016,761237000430,402232601826,402232700450,768235400069,402234000837,402234500128,403232800168,403232900931,346234502486,768236100461,769236501286,771236100213,405234500162,405234600927,330234600142,743237001253,106718614713)
                    and b.order_id <> ''
                    group by a.uuid
                    order by b.order_id""")
                        ...
                        if qty_delivered != 0:
                            ...
                            for qty in range(qty_delivered):
                                ...
                                if xml_files:
                                    ...
                                    invoices_folder = 'G:/Mi unidad/xml_sr_mkp_invoices/Julio/'
                            ...
                            for qty in range(qty_uom):
                                ...
                                if xml_files:
                                    ...
                                    file_name_mayus = file_name.upper()
                                    print(f"AGREGANDO ARCHIVO XML A LA FACTURA")
                                    invoices_folder = 'G:/Mi unidad/xml_sr_mkp_invoices/Julioº/'
                                    print(f"El xml {file_name} será agregado a la factura")
                                    xml_file = file_name + '.xml'
                                    xml_file_path = os.path.join(invoices_folder, xml_file)
                                    with open(xml_file_path, 'rb') as f:
