...
def invoice_create_qty():
    ...
    mycursor.execute("""select b.name, a.uuid, a.fecha
                        ...
                        WHERE extract(year_month from a.fecha) = 202310
                            ...
                            and a.folio in (848236702230,849236801425,850236900413,855236501360,856234800553,858235300545,860234900614,862235200272,864237100649,865235400239,865235400497,865236801388,866234900665,866236400392,866236600246,866236601453)
                        group by a.uuid
                        order by a.fecha
                        """)
