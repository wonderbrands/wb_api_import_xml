...
print('-------------------------------------------------------')
mycursor.execute("SELECT so_name, uuid FROM sr_so_global_invoice")
                ...
                if qty_delivered != 0:
                    for qty in range(qty_delivered):
                        ...
                        if uuid_reference:
                            ...
                            xml_file = uuid_reference + '@1000000000XX0.xml'
                            invoices_folder = dir_path + '/xml/'
                            print(f"El xml {xml_file} será agregado a la factura")
